...
func TestOnAndTrigger(t *testing.T) {
	tests := []test{
		{name: "Simple", f: TriggerOn_Simple, want: 1},
	...
	t.Run(
		"Outer test", func(t *testing.T) {
			for _, tst := range tests {
				curTest := tst
				t.Run(
					curTest.name, func(t *testing.T) {
						t.Parallel()
...
func TriggerOn_NoEventsTriggerWithChannel(
	ctx context.Context,
	t *testing.T,
	name string,
...
func TriggerOn_NoEventsTrigger(
	ctx context.Context,
	t *testing.T,
	name string,
	_ func(ctx context.Context) string,
...
func TriggerOn_Simple(
	ctx context.Context,
	t *testing.T,
	triggerName string,
	farg func(ctx context.Context) string,
) string {
	var (
		eventDefault, _ = event.NewEvent(
			event.Args{
				Fun:         farg,
...
func TriggerOn_MultipleDefaultAndOnce(
	ctx context.Context,
	t *testing.T,
	triggerName string,
	farg func(ctx context.Context) string,
	...
	var (
		...
		execCh       = make(chan string)
		errG, errCtx = errgroup.WithContext(ctx)
		testCtx, _   = ctxWithValueAndTimeout(
			errCtx, internal.EXEC_CH_CTX_KEY, execCh,
			time.Second,
		)
	)
